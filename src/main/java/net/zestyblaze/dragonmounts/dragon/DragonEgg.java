...
//    private boolean wiggling;
    ...
    public DragonEgg(EntityType<DragonEgg> type, Level level) {
        super(type, level);
...
//        wiggleTime = LerpedFloat.unit();
    ...
    public DragonEgg(Level level) {
        super(DMLRegistry.DRAGON_EGG, level);
        breed = BreedManager.getFallback();
...
//        wiggleTime = LerpedFloat.unit();
    ...
    public void onSyncedDataUpdated(EntityDataAccessor<?> key) {
        if (key.equals(BREED)) breed = BreedManager.read(entityData.get(BREED));
            ...
            DragonBreed breed = BreedManager.getNullable(ResourceLocation.tryParse(entityData.get(TransitionHandler.TRANSITION_BREED)));
...
//            if (wiggleTime.get() == 1) wiggling = false;
    ...
    public void addHatchingParticles() {
        ...
        ParticleOptions particle = breed.getHatchParticles(random);
...
//            level.globalLevelEvent(2001, blockPosition(), Block.getStateId(breed.getDefaultState()));
            ...
            TameableDragon dragon = DMLRegistry.DRAGON.create(level);
...
//            this.wiggling = true;
    ...
    public class TransitionHandler {
        ...
        public void read(CompoundTag tag) {
            DragonBreed breed = BreedManager.getNullable(ResourceLocation.tryParse(tag.getString(NBT_TRANSITION_BREED)));
