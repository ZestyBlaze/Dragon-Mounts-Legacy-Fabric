...
public class GreenToesAbility extends FootprintAbility {
    ...
    protected void placeFootprint(TameableDragon dragon, BlockPos pos) {
        ...
        if(steppingOn.is(BlockTags.MUSHROOM_GROW_BLOCK)) {
            ...
            placing = Registry.BLOCK.getTag(BlockTags.SMALL_FLOWERS).flatMap(tag -> tag.getRandomElement(dragon.getRandom())).map(Holder::value).filter(b -> b != Blocks.WITHER_ROSE).orElse(Blocks.DANDELION).defaultBlockState();
        }

        if(placing != null && placing.canSurvive(level, pos)) {
            level.setBlockAndUpdate(pos, placing);
            ((ServerLevel) level).sendParticles(ParticleTypes.HAPPY_VILLAGER, pos.getX(), pos.getY(), pos.getZ(), dragon.getRandom().nextInt(4) + 2, 0.5, 0.5, 0.5, 1);
        }
    }
